...
df = pd.read_csv('waveform_export_COS6.csv')
time       = df['time_s'].values
raw_signal = df['Ant.-Post. [millimeter]'].values

# percentile 
p_high = np.percentile(raw_signal, 99)  # αντί max
p_low  = np.percentile(raw_signal, 1)   # αντί min
midpoint = (p_high + p_low) / 2
centered = -(raw_signal - midpoint)

# LOW-PASS FILTER
fs   = 1 / (time[1] - time[0])  # 25 Hz
b, a = butter(4, 1.0 / (fs/2), btype='low')
filtered = filtfilt(b, a, centered)

nominal_amplitude = 10.0
nominal_frequency = 0.125  

best_phase, best_exp, best_rms = 0, 6, np.inf

for exp in [4, 5, 6, 7, 8, 10, 12]:
    for phase in np.linspace(0, 2*np.pi, 500):
        cos_raw      = np.abs(np.cos(2 * np.pi * nominal_frequency * time + phase)) ** exp
        cos_centered = cos_raw - np.mean(cos_raw)
        pos_max = np.max(cos_centered)
        neg_min = np.min(cos_centered)
        theo = np.where(
            cos_centered >= 0,
            nominal_amplitude * cos_centered / pos_max,
            nominal_amplitude * cos_centered / abs(neg_min)
        )
        rms = np.sqrt(np.mean((filtered - theo)**2))
        if rms < best_rms:
            best_rms, best_phase, best_exp = rms, phase, exp

print(f"Best Exponential: {best_exp}")
print(f"Best Phase:     {best_phase:.4f} rad")
print(f"Best RMS:      {best_rms:.2f} mm")

cos_raw      = np.abs(np.cos(2 * np.pi * nominal_frequency * time + best_phase)) ** best_exp
cos_centered = cos_raw - np.mean(cos_raw)
pos_max      = np.max(cos_centered)
neg_min      = np.min(cos_centered)
cos6_scaled  = np.where(
    cos_centered >= 0,
    nominal_amplitude * cos_centered / pos_max,
    nominal_amplitude * cos_centered / abs(neg_min)
)
phase_shift = best_phase
...
error_sig  = filtered - cos6_scaled
...
# 1. Raw vs Filtered
axes[0].plot(time, centered,  label='Raw',
             alpha=0.3, color='gray', linewidth=0.8)
axes[0].plot(time, filtered,  label='Filtered (<1Hz)', color='#1f77b4', linewidth=1.5)
axes[0].axhline( nominal_amplitude, color='orange', linewidth=0.8,
                linestyle='--', label=f'±{nominal_amplitude}mm')
...
# 2. Comparison
axes[1].plot(time, filtered,    label='Nominal (filtered)', color='#1f77b4', linewidth=1.5)
axes[1].plot(time, cos6_scaled, label=f'Theoretical cos⁶  (A=±{nominal_amplitude}mm, T=4s)',
...
# 3. Error
axes[2].plot(time, error_sig, color='green', linewidth=1.0, label='Error')
...
axes[0].axhline(np.mean(errors_per_cycle), color='red', linestyle='--',
                linewidth=1.5, label=f'Mean RMS = {np.mean(errors_per_cycle):.2f} mm')
...
axes[0].set_title('RMS Error per breathing cycle — cos⁶')
axes[0].set_xlabel('Breathing cycle (#)')
...
axes[1].axvline(np.mean(errors_per_cycle), color='red', linestyle='--',
                linewidth=1.5, label=f'Μέσος = {np.mean(errors_per_cycle):.2f} mm')
axes[1].axvline(np.max(errors_per_cycle), color='orange', linestyle=':',
                linewidth=1.5, label=f'Max = {np.max(errors_per_cycle):.2f} mm')
axes[1].set_title('Κατανομή RMS Σφάλματος ανά Κύκλο — cos⁶')
axes[1].set_xlabel('RMS Σφάλμα (mm)')
axes[1].set_ylabel('Αριθμός κύκλων')
axes[1].legend()
axes[1].grid(True)
...
print(f"\nAnalysis per cycle ({len(errors_per_cycle)} cycles):")
...
print(f"Error Amplitude:     {error_amp:.2f}%")
print(f"RMS Error:      {rms_error:.2f} mm")
# Real Period Measurement (Like POLIZZI)
